...
이 문서는 Capston의 원천 데이터, 업데이트 정책, 프론트엔드 제공 API를 정리합니다. 파일 기반 데이터의 세부 출처는 `backend/data/DATA_SOURCES.md`를 함께 봅니다.
...
| `stores` | API + 파일 | 소상공인 API, 업종분류 XLSX, KSIC XLSX | `store`, `business_category`, `ksci_category` | 전체 성공 시 삭제 반영 |
...
현재 코드에 존재하는 주요 API입니다.

| API | 용도 |
|---|---|
| `GET /api/adongs/scores` | 지도/목록용 행정동 점수 |
| `GET /api/adongs/match-counts` | 조건 매칭 개수 |
| `GET /api/adongs/<slug>/match-detail` | 조건 매칭 상세 |
| `GET /api/adongs/<slug>/summary` | 동네 요약 패널 |
| `GET /api/adongs/<slug>/detail` | 동네 상세 화면 |
| `GET /api/adongs/<slug>/explore` | 조건 기반 탐색 |
| `GET /api/adongs/<slug>/population` | 인구 지표 |
| `GET /api/adongs/<slug>/gu-metrics` | 구 단위 지표 |
| `GET /api/adongs/<slug>/parks` | 공원 목록 |
| `GET /api/adongs/<slug>/transit-congestion` | 교통 혼잡도 |
| `GET /api/adongs/<slug>/gu-metrics/series` | 구 지표 시계열 |
| `GET /api/adongs/<slug>/derived-indices` | 자취촌 지수와 계약 활발도 |
| `GET /api/compare` | 동네 비교 |
| `POST /api/score/point` | 특정 좌표 점수 계산 |
| `GET /api/preference/pairs` | 선호 학습 비교 쌍 |
| `POST /api/preference/submit` | 선호 학습 제출 |
...
| `GET/PUT /api/users/me/preference` | 내 선호 설정 |
