...
- SubwayStation: 이전 `adong FK`(neighborhoods.Adong) 제거 → adong/ldong FK 정합.
...
sub-plan 7G-C (결정 4A):
- 이전 NearestSubway(neighborhoods.Adong FK) 모델 및 nearest_subway 테이블 완전 폐기.
...
#
# 이전 NearestSubway(neighborhoods.Adong FK) 폐기.
