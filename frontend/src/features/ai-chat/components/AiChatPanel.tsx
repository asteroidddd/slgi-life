...
export default function AiChatPanel({
  ...
  return (
    <div className={`fixed inset-0 z-[1680] pointer-events-none ${isOpen ? '' : 'hidden'}`} aria-hidden={!isOpen} aria-live="polite">
      ...
      >
        <header className="flex items-center justify-between gap-4 border-b border-divider px-5 py-4">
          <div className="min-w-0">
            <p className="m-0 text-[12px] font-bold text-primary">AI</p>
            <h2 className="m-0 mt-1 truncate text-[20px] font-semibold leading-tight">
              {compareCandidates ? '담은 동네 비교하기' : '대화로 동네 찾아보기'}
            </h2>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="h-9 w-9 shrink-0 rounded-[8px] bg-surface-alt text-[20px] font-bold leading-none text-text-muted transition hover:text-text"
            aria-label="AI 채팅 닫기"
          >
            ×
          </button>
