...
const CHART_TOOLTIP_CLASS = 'app-tooltip app-tooltip--compact pointer-events-none absolute z-[var(--tooltip-z)] hidden min-w-[136px] text-center group-hover:block group-focus-within:block';
...
function InfoButton({ text }: { text: string }) {
  return (
    <span className="group relative z-[80] inline-flex">
      ...
      <span className="app-tooltip app-tooltip--info app-tooltip--muted pointer-events-none absolute right-0 top-[calc(100%+6px)] z-[var(--tooltip-z)] hidden w-[300px] whitespace-pre-line text-left group-hover:block group-focus-within:block">
        {text}
      </span>
    </span>
  );
      ...
      {items.map((item, index) => {
        ...
        return (
          <div key={`${item.month ?? item.time ?? index}`} className="group relative flex h-full flex-1 flex-col items-center justify-end gap-2">
            ...
            <span className="app-tooltip app-tooltip--compact pointer-events-none absolute bottom-[calc(100%+8px)] left-1/2 z-[var(--tooltip-z)] hidden min-w-[136px] -translate-x-1/2 text-center group-hover:block">
              {label}<br />
    ...
    <div className="grid gap-2">
      {top.map((item) => {
        ...
        <div key={`${item.category ?? item.housing_type ?? item.label}`} className="group relative grid grid-cols-[86px_1fr_44px] items-center gap-2 text-[12px]">
          ...
          <span className="app-tooltip app-tooltip--compact pointer-events-none absolute right-0 top-[calc(100%+6px)] z-[var(--tooltip-z)] hidden group-hover:block">
...
function normalizeInfraText(text?: string) {
  if (!text) return '';
  return text
    .replace(/응급실은 주변 지역까지 함께 봐야 합니다/g, '병원·약국 접근성을 함께 확인하세요')
    .replace(/응급실은 \d+곳이고, 오늘 운영 중인 의료시설은 ([0-9,]+)곳입니다\./g, '오늘 운영 중인 병원·약국은 $1곳입니다.')
    .replace(/응급실은 [^.。]*[.。]?/g, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
}

function normalizeInfraQuicktakes(items?: QuickTake[]) {
  return items
    ?.filter((item) => !item.label.includes('응급실'))
    .map((item) => ({ ...item, label: normalizeInfraText(item.label) || item.label }));
}
...
function SafetyRadarChart({ items }: { items: SafetyGradeItem[] }) {
  ...
  return (
    <div className="relative grid min-h-[190px] place-items-center">
      ...
      {hoveredItem ? (
        <div className="app-tooltip app-tooltip--compact pointer-events-none absolute left-1/2 top-2 z-[var(--tooltip-z)] min-w-[170px] -translate-x-1/2 text-center">
...
function SafetyWmsMap({ regionLevel, slug, meta }: { regionLevel: RegionLevel; slug: string | null; meta?: SafetyWmsLayerResponse }) {
  ...
  return (
    <div className="group relative h-full min-h-[340px] overflow-hidden rounded-[var(--map-control-radius)] border border-border bg-surface-alt">
      <div className="app-tooltip app-tooltip--info app-tooltip--muted pointer-events-none absolute right-3 top-3 z-[var(--tooltip-z)] hidden w-[270px] group-hover:block">
  ...
  return (
    <div className="mt-5 grid gap-4" data-region={basePath ?? ''}>
      ...
      <section className="rounded-card border border-border bg-surface p-5 shadow-sm">
        <SectionHeader
          kicker="생활 인프라"
          title={infraHeadline || `면적당 시설은 ${metricValue(pickMetric(infraMetrics, 'amenity_density'))} 수준입니다`}
          summary={infraSummary || '식생활, 문화, 학습, 의료 시설을 서울 평균과 함께 봅니다.'}
          info={infraInfo}
        >
          <SummaryCluster quicktakes={infraQuicktakes} />
        </SectionHeader>
        <MetricGrid items={[
