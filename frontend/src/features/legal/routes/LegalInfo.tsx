import { BackButton } from '@/features/common/components/AppChrome';
    ...
    articles: [
      ...
      },
      {
        ...
        items: [
          ...
          '담은 동네: 사용자가 비교하거나 다시 보기 위해 저장한 행정동 또는 법정동 후보를 말합니다.',
      ...
      },
      {
        ...
        items: [
          ...
          '조건 입력을 통한 동네 추천과 추천 결과 조회',
          '전월세 실거래 기반 시세 요약, 환산월세 추이, 거래량 표시',
          '교통 접근성, 생활 인프라, 안전 지표 등 지역 대시보드',
          '네이버페이 부동산, 직방, 다방 등 외부 매물 사이트로 이동하는 실제 매물 보기 링크',
          '부동산 계약 체크리스트, 매물 주소·면적·유형·보증금·월세를 입력받아 주변 거래와 비교하는 매물 분석, 외부 확인 링크를 제공하는 부동산 도우미',
          '사용자 설정과 지역 데이터를 바탕으로 답변을 생성하는 AI에게 묻기 기능',
          '회원 프로필, 집 위치, 담은 동네, 선택 조건, AI API 키 관리 기능',
        ],
      },
      {
        title: '제5조 회원가입과 로그인',
      ...
      },
      {
        ...
        items: [
          ...
          '사용자는 실제 매물 보기로 이동한 외부 사이트의 이용약관과 개인정보처리방침을 직접 확인해야 합니다.',
      ...
      },
      {
        ...
        items: [
          ...
          '조건 추천 결과는 사용자가 입력한 예산, 통학, 선호시설, 우선순위와 서비스 DB를 기준으로 계산한 참고 후보이며, 실제 주거 적합성을 보장하지 않습니다.',
          ...
          '실제 매물 보기 링크는 외부 사이트의 검색 화면으로 이동시키는 기능이며, 자취맵이 외부 매물의 존재, 가격, 계약 가능 여부, 표시 정확성을 보장하지 않습니다.',
        ],
      },
      {
        ...
        items: [
          ...
          '서비스의 외부 링크와 실제 매물 보기 링크는 사용자의 추가 확인을 돕기 위한 것이며, 외부 사이트의 정보와 서비스 운영에 대한 책임은 각 제공자에게 있습니다.',
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          ...
          '회원탈퇴 시 계정, 프로필, 집 주소와 좌표, 담은 동네, 저장된 선택 조건, 저장된 AI API 키 등 서비스 제공에 필요한 회원 관련 정보가 삭제됩니다.',
    ...
    articles: [
      {
        ...
        items: [
          ...
          '집 위치 이동, 집 마커 표시, 담은 동네 등 개인화 기능 제공',
          '담은 동네 저장과 조건 기반 동네 추천 결과 제공',
          '매물 분석 요청 처리와 실제 매물 보기 외부 링크 생성',
      ...
      },
      {
        ...
        items: [
          ...
          '관심 지역 정보: 담은 동네로 저장한 행정동 또는 법정동',
          '조건 추천 정보: 예산, 주거 면적, 통학 대상 대학, 통학 허용 시간, 선호시설, 우선순위 등 사용자가 입력하거나 선택한 조건',
          '매물 분석 입력 정보: 분석을 위해 사용자가 입력한 매물 주소, 전용면적, 주거 유형, 보증금, 월세, 인접 지역 비교 여부',
      ...
      },
      {
        ...
        items: [
          ...
          '이메일, 학교, 입학연도, 집 주소, 담은 동네, 선택 조건, 매물 분석 입력값, AI API 키, AI 맥락 공유 설정은 사용자가 서비스 화면에서 직접 입력하거나 저장합니다.',
      ...
      },
      {
        ...
        items: [
          ...
          '네이버페이 부동산, 직방, 다방: 사용자가 실제 매물 보기 링크를 클릭하면 선택한 동네, 좌표, 줌 수준, 주택유형 등 링크 구성에 필요한 값이 URL에 포함되어 외부 사이트로 이동할 수 있습니다.',
      ...
      },
      {
        ...
        items: [
          ...
          '회원탈퇴 시 계정, 프로필, 집 주소와 좌표, 담은 동네, 저장된 선택 조건, 저장된 AI API 키 등 회원 관련 정보는 삭제됩니다.',
          '매물 분석 입력 정보와 실제 매물 보기 링크 클릭은 원칙적으로 계정 저장 정보로 보관하지 않으며, 오류 대응에 필요한 서버 로그에는 요청 시각과 오류 정보가 남을 수 있습니다.',
      ...
      },
      {
        title: '제8조 이용자의 권리',
        items: [
          '사용자는 마이페이지에서 이메일, 프로필, 집 주소, AI API 키, AI 맥락 공유 설정을 조회, 수정, 삭제할 수 있으며, 화면 기능을 통해 담은 동네를 추가하거나 삭제할 수 있습니다.',
    ...
    articles: [
      {
        ...
        rows: [
          ...
          },
          {
            category: '조건 추천',
            source: '카카오',
            usage: '조건 추천에서 선택한 대학까지의 통학 가능 여부와 정렬 기준을 계산하는 데 사용합니다. 행정동·법정동 대표 좌표와 대학 좌표를 기준으로 카카오맵 길찾기 결과를 활용하였습니다.',
            links: [
