...
서비스 URL: https://slgi-life.duckdns.org/

## 서비스 흐름

| 단계 | 화면 | 역할 |
|---|---|---|
| 시작 | `/recommend/conditions` | 예산, 통학, 선호 시설, 우선순위를 입력 |
...
| 후보 비교 | 우측 후보 패널 | 담은 동네를 비교하고 최종 후보에서 자세히 보기 |
| 지도 탐색 | `/map` | 부가 지도 서비스. 시설, 의료, 점수 히트맵을 보며 동네 선택 |
| 부동산 도우미 | `/real-estate` | 체크리스트, 매물 분석, 외부 링크 제공 |
| AI | 우측 사이드 패널 | 현재 화면을 유지한 채 AI 채팅 열기 |

`/select`는 호환 경로이며 `/recommend/conditions`로 이동합니다. `/ai-chat`은 AI 패널 진입용 호환 경로입니다.
...
- 서버 `.env`의 `AI_AGENT_OPENAI_API_KEY` 기반 AI 질의. 동네 간단 요약은 대시보드 캐시를 우선 활용하고, 상세 근거는 허용된 공공데이터 원천 테이블을 조회
...
│   │   ├── ai_agent/       # AI 질의, 내부 OpenAI 키 사용, SQL guard
│   │   ├── caches/         # 전월세 지오코딩, 지역 통계, 지도 마커 운영 캐시
...
| 경로 | 역할 |
|---|---|
| `/` | `/recommend/conditions`로 이동 |
| `/select` | `/recommend/conditions`로 이동하는 호환 경로 |
...
| `GET /api/compare/neighborhoods` | 후보 동네 비교 |
| `GET /api/compare/commute-time` | 후보 동네 통학 시간 비교 |
...
| `GET/PATCH /api/agent/context-preferences` | AI 맥락 공유 설정 |
| `DELETE /api/agent/conversation/<conversation_id>` | AI 대화 초기화 |
...
| `POST /api/auth/kakao/webhook` | Kakao webhook |
